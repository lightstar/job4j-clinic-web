...
/**
 * <code>DeleteRoleTest</code> servlet tests.
 *
 * @author LightStar
 * @since 0.0.1
 */
public class DeleteRoleTest extends ServletTest {
    ...
    public void whenDoPostThenResult() throws ServiceException, ServletException, IOException {
        ...
        new DeleteRole(this.clinicService, this.roleService).doPost(this.request, this.response);

        verify(this.session, atLeastOnce()).setAttribute(eq("message"), anyString());
        verify(this.roleService, times(1)).deleteRole("client");
    ...
    /**
     * Test deleting role when role service throws exception.
     */
    @Test
    public void whenDoPostWithServiceExceptionThenError()
            throws ServletException, IOException, ServiceException {
        when(this.request.getParameter("name")).thenReturn("client");
        doThrow(new ServiceException("Test error")).when(this.roleService).deleteRole("client");
